...
module Gameplay
  class QuantumMovesCalculator
    ...
    class << self
      def all_beat_move_steps(boards, player, ruleset: Gameplay::RussianRuleset)
        all_possible_move_steps(boards, player, ruleset: ruleset).select(&:beat?)
      end

      def any_can_beat?(boards, player, ruleset: Gameplay::RussianRuleset)
        boards.any? do |board|
          Gameplay::MovesCalculator.any_can_beat?(board, player, ruleset: ruleset)
        end
      end

      def all_possible_move_steps(boards, player, ruleset: Gameplay::RussianRuleset)
        boards.flat_map do |board|
          Gameplay::MovesCalculator.all_possible_move_steps(board, player, ruleset: ruleset)
        end
      end

      def all_possible_move_chains(boards, player, ruleset: Gameplay::RussianRuleset)
        boards.flat_map do |board|
          Gameplay::MovesCalculator.all_possible_move_chains(board, player, ruleset: ruleset)
        end
      end

      def valid_possible_move_chains(boards, player, ruleset: Gameplay::RussianRuleset)
